...
def play
 ...
 class ComputerPlayer < Player
   DEBUG = false # edit if needed later
   ...
   def select_position!
     ...
     if corner_trap_defense_needed?
      ...
      #could sometimes do corner trap  offense
  ...
  def corner_trap_defense_needed?
    ...
    opponent_chose_a_corner = corner_positions.any? {
      |pos| @game.board[pos] != nil }
      return @game.turn_num == 2 && opponent_chose_a_corner
    end

    def corner_trap_defense_position(opponent_marker)
      #if you move in center or opposite corner, opponent might force you to lose
      log_debug "defending against corner start by playing adjacent"
      #playing in adjacent corner is safe, but require more logic later
